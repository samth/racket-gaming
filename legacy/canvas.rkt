...
(require "constants.rkt"
         ...
         (rename-in gaming/all
                    [make-color make-native-color]))
...
(define-generics*
  ...
  (game-dc% set-pen set-brush set-font set-text-foreground
  ...
  (let ((image
         (read-bitmap
          (if (file-exists? path)
              path
              (begin
                (display "the image in \"~a\" could not be loaded: file not found" path)
                (newline)
                "unknown.png"))
          (cond
...
(define (on-loose! proc)
  (chain mouse left release listeners (add! proc)))
...
(define (off-loose! proc)
  (chain mouse left listeners (remove! proc)))
